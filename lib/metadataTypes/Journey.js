...
class Journey extends MetadataType {
    ...
    /**
     * a function to pause journey execution via API
     *
     * @param {TYPE.MetadataTypeItem[]} ids array of ids or a map of ids and journey names
     * @returns {Promise.<string[]>} Returns list of keys that were paused
     */
    static async pause(ids) {
        const requestBody = {
            ExtendWaitEndDates: false,
            PausedDays: 1,
            ProcessWaitUntilEvents: true,
            GuardrailAction: 'Resume',
            RetainContactInjectionWhileJourneyPaused: true, //queue and process contacts
        };
        const results = [];
        let idArr = Array.isArray(ids) ? ids : Object.keys(ids);
        for (const id of idArr) {
            let journeyName = Array.isArray(ids) ? '' : ` - ${ids[id]}`;
            Util.logger.info(`Pausing ${this.definition.type} ${id} ${journeyName}`);
            const response = await this.client.rest.post(
                `/interaction/v1/interactions/pause/${id}?allVersions=true`,
                {}
            );
            if (response.status == 'Accepted') {
                Util.logger.info(` - paused ${this.definition.type}: ${id}${journeyName}`);
                results.push(id);
            } else {
                Util.logger.warn(
                    `Could not pause ${this.definition.type}: ${id}.${response.message}`
                );
            }
        }
        return results;
     ...
     * @returns {Promise.<string[]>} Returns list of keys that were paused
     */
    static async execute(ids) {
        const results = [];
        let idArr = Array.isArray(ids) ? ids : Object.keys(ids);
        for (const id of idArr) {
            let journeyName = Array.isArray(ids) ? '' : ` - ${ids[id]}`;
            Util.logger.info(`Resuming ${this.definition.type} ${id}${journeyName}`);
            const response = await this.client.rest.post(
                `/interaction/v1/interactions/resume/${id}?allVersions=true`,
                {}
            );
            if (response.status == 'Accepted') {
                Util.logger.info(` - resumed ${this.definition.type}: ${id}${journeyName}`);
                results.push(id);
            } else {
                Util.logger.warn(
                    `Could not resume ${this.definition.type}: ${id}.${response.message}`
                );
            }
        }
        return results;
