    ...
    static async #retrieveKeysWithLike(selectedType, buObject) {
        ...
        for (const originalKey in metadataMap) {
            ...
            if (Util.fieldsLike(metadataMap[originalKey])) {
                keyNameMap[metadataMap[originalKey].id] = metadataMap[originalKey].name;
            }
        }
        Util.logger.info(
            Util.getGrayMsg(
                `Identified ${Object.keys(keyNameMap).length} ${selectedType}${
                    Object.keys(keyNameMap).length === 1 ? '' : 's'
                } that match${Object.keys(keyNameMap).length === 1 ? 'es' : ''} the like-filter`
            )
        );
        return keyNameMap;
    }
